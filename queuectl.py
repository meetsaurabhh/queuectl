#!/usr/bin/env python3
import argparse,sys
from db import init_db, add_default_config, add_job, gen_id, list_jobs, get_counts, get_metrics
from worker import start_workers

def cmd_hello(): print("queuectl is set up.")
...
def cmd_status(_): c=get_counts();[print(f"{k}: {v}") for k,v in c.items()]
def cmd_worker(a):
    if a.action=="start": print(f"Starting {a.count} workers..."); start_workers(a.count)
...
def cmd_dlq(a):
    if a.action=="list":
        ...
        ok=retry_dead(a.job_id); print("Retried" if ok else "Job not found or not dead.")
def cmd_stats(_):
    m=get_metrics()
    print("Metrics:")
...
def main():
    ...
    sp.add_parser('hello'); sp.add_parser('initdb')
